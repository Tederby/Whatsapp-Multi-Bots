...
- **Empirical Webview Testing Suite (`!wvtest`)**: Implemented `commands/wvtest.js` featuring in-webview automated capability matrix (35+ browser checks), calibrated stanza size probes (exact byte matching for finding drop thresholds), protobuf envelope mutation testing (`botjid`, `multisection`, `trusted_sources`, `contextInfo`), audio decoding probes, URI scheme interception audits, and lifecycle persistence checks.
- **Dedicated Webview Specification (`WEBVIEW_PAYLOAD.md`)**: Modularized webview documentation into a dedicated technical specification (`docs/WEBVIEW_PAYLOAD.md`) serving both as a universal reference for Baileys bot developers and an architectural guide for this project's `lib/uiEngine.js`. Streamlined `docs/COMMAND_DEVELOPMENT.md` and updated documentation links across `docs/ARCHITECTURE.md`, `docs/README.md`, and root `README.md`.
