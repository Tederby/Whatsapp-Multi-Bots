...
- **Web Audio `decodeAudioData` Breakthrough**: Discovered that while HTML5 `<audio>` elements are quarantined (`NotSupportedError: The Element has no supported sources`), converting Base64 audio into an `ArrayBuffer` and decoding via Web Audio API (`ctx.decodeAudioData()`) **successfully decodes and plays audio** through an `AudioBufferSourceNode` inside the webview.
- **Protobuf Multi-Card & Clean Envelope Discoveries**:
  - **Native Multi-Card Support**: The `sections` array in `unifiedResponse.data` natively supports multiple `GenAISingleLayoutViewModel` items, rendering multiple HTML cards stacked inside a single WhatsApp message bubble.
...
- **Modular Testing Suite & Extended Probes (`_wvtestTemplates.js`)**: Extracted all HTML/CSS/JS template generators from `commands/wvtest.js` into companion module `commands/_wvtestTemplates.js` (ignored by command registry via `_` prefix), reducing `wvtest.js` controller from 1,051 to 486 lines. Expanded automated suite with 50+ checks including real IndexedDB read/write transactions, real WebAssembly compilation/execution, outbound Fetch CORS & Localhost/SSRF probes, live WebSocket handshake probes, deep `window` property scanner, HTML5 `<video>`, and FontFace API. Added interactive form & soft keyboard viewport watcher (`!wvtest form`), and expanded protobuf mutations with `quotedMessage` context reply and native ephemeral disappearing timer (`!wvtest proto quoted` & `!wvtest proto expiration`).
- **Air-Gapped Sandbox, IndexedDB Denial & Native Form Findings**: Finalized 43-point capability matrix (25 Pass, 18 Blocked). Proved that runtime outbound `fetch()` and `WebSocket` connections are strictly blocked by client sandbox rules (air-gapped container). Confirmed that `indexedDB.open()` throws `SecurityError` due to `about:blank` opaque origin, enforcing zero-disk persistence. Confirmed that `WebAssembly.instantiate()` is blocked by CSP lacking `'wasm-unsafe-eval'`. Confirmed native mobile `<select>`, `<input type="date">`, `<input type="color">`, and soft keyboard downward pushing, while `<input type="file">` is completely unhandled (`onShowFileChooser` missing). Confirmed that `contextInfo.quotedMessage` successfully renders native reply quotes above webview cards.
- **Dedicated Webview Specification (`WEBVIEW_PAYLOAD.md`)**: Modularized webview documentation into a dedicated technical specification (`docs/WEBVIEW_PAYLOAD.md`) serving both as a universal reference for Baileys bot developers and an architectural guide for this project's `lib/uiEngine.js`.
