...
const stmts = {
    ...
    // Sider Tracking
    incrementMsgCount: db.prepare(`
        INSERT INTO group_message_counts (group_id, user_id, count)
        VALUES (?, ?, 1)
        ON CONFLICT(group_id, user_id) DO UPDATE SET count = count + 1
    `),
    getGroupMsgCounts: db.prepare(`
        SELECT user_id, count FROM group_message_counts
        WHERE group_id = ? ORDER BY count DESC
    `),
    getGroupSiders: db.prepare(`
        SELECT user_id, count FROM group_message_counts
        WHERE group_id = ? AND count < ? ORDER BY count ASC
    `),
    resetGroupCounts: db.prepare(`
        DELETE FROM group_message_counts WHERE group_id = ?
    `),
    getUserMsgCount: db.prepare(`
        SELECT count FROM group_message_counts
 ...
 * Uses UPSERT — single SQL statement for performance.
 */
export function incrementMsgCount(groupId, userId) {
    stmts.incrementMsgCount.run(groupId, userId);
}

/**
 * Get all user message counts in a group, sorted by count DESC.
 * @returns {Array<{user_id: string, count: number}>}
 */
export function getGroupMsgCounts(groupId) {
    return stmts.getGroupMsgCounts.all(groupId);
}

/**
 * Get users in a group whose message count is below threshold.
 * @returns {Array<{user_id: string, count: number}>}
 */
export function getGroupSiders(groupId, threshold) {
    return stmts.getGroupSiders.all(groupId, threshold);
}

/**
 * Delete all message count data for a group.
 */
export function resetGroupMsgCounts(groupId) {
    stmts.resetGroupCounts.run(groupId);
}

/**
 * Get individual user's message count in a group.
 * @returns {number}
 */
export function getUserMsgCount(groupId, userId) {
    const row = stmts.getUserMsgCount.get(groupId, userId);
...
export function repairDatabase() {
    ...
    const tx = db.transaction(() => {
        ...
        // 6. Clean orphan tracking data (group no longer exists in groups table)
        const orphanTracking = db.prepare(`
            DELETE FROM group_message_counts 
            WHERE group_id NOT IN (SELECT id FROM groups)
        `).run();
        report.orphanTracking = orphanTracking.changes;
    });
    tx();
    ...
    try {
            ...
            for (const file of pfpFiles) {
                ...
                if (!user || user.meta?.pfp !== file) {
                    try {
                        ...
                        logger.warn("DBFIX", `Gagal menghapus orphan PFP (${file}): ${e.message}`);
                    }
                }
            }
        ...
        logger.error("DBFIX", `PFP sync error: ${err.message}`);
    }
